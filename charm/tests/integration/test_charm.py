...
from lightkube.models.core_v1 import PodSpec, Container, ContainerPort
...
async def set_application_config(application, config: dict, wait_time=10):
    """Updates the config of an application and waits for the settings to take effect.
...
async def test_webhook_workload(ops_test: OpsTest, temp_pod_deleter):
    ...
    test_pod_created = lightkube_client.create(test_pod, name=test_pod.metadata.name, namespace=ops_test.model.name)
